---
type: "event"
title: "Djibian Talk + Public Workshop"
publishDate: "2026-06-04"
date: "2026-06-05T14:30:00"
draft: false
bg_image: "images/backgrounds/party-event.jpg"
description: "Free yourself and take back control with Djibian and OpenPGP security keys"
image: "images/events/2026/20260605_djibian_atelier.jpg"
location: "[Tallard 🇫🇷](https://www.openstreetmap.org/way/662873510)"
audience: "Open to all."
fee: "Free"
apply_url: "tel:+33756832215"
lang: "en"

---

# Come discover and take charge of your digital future

*Because your **PERSONAL** data is a treasure, and **FREEDOM** your essence.*

**[Foopgp](/about/)** has selected and developed a set of tools, integrated into a GNU/Linux operating system (*Debian Blends*) called **[Djibian](/blog/2025-09-12-welcome-djibian/)**.

**[Djibian](/blog/2026-01-19-djibian-release/)** is today the most advanced system for using OpenPGP security keys, such as the YubiKey 5 NFC or the NitroKey 3A NFC.

In other words: **[Djibian](/blog/2026-02-10-djibian-tour/)** is **the consumer operating system most respectful of your human singularity, and the safest.**

---

### Meet us on Friday 5 June in Tallard, to

1. **Understand** why it matters to build and share these new tools.
2. **Dare to give birth to your [OpenPGP ID](/about/openpgp/) identity**, secure it with your **[YubiKey or NitroKey](/solutions/offer-security-keys/)**, and use it from your computer or smartphone.
3. **Discover the power of the [dji (Ɉ)](/about/join/) token**, the social share of our digital-sovereignty society.

![OpenPGP security keys: NitroKey, YubiKey, and others](/images/solutions/OpenPGPkeys.jpg)

This way you will begin to genuinely take back control of your data — data that, otherwise, currently lets unscrupulous foreign corporations control *you*.

### Install party

We will provide a few refurbished computers running [Djibian](/blog/2026-02-10-djibian-tour/), which you can buy in euros (€) [or in djis (Ɉ)](https://foopgp.org/fr/about/join/).

If you have computers that crawl, glitch or spy on you (any Windows or macOS box, for instance), **we'll also help you disinfect them** by installing Djibian on them. *(Remember to back up your data beforehand.)*

---

## Programme

#### --- 14:30 --- Djibian talk
- Concepts and terminology: **Linux**, **OpenPGP**, **YubiKey**, **Libre Currency**, **dji (Ɉ)**.

#### --- 16:00 --- Djibian workshop
- Boot your machine.
- Create and print your **[OpenPGP ID](/solutions/openpgp-id/)** identity.
- Set up the **[YubiKey or NitroKey](/solutions/offer-security-keys/)** that will secure your singularity in the digital world.
- Use that key to:
  - Create your account on **[Djibian](/blog/2026-02-10-djibian-tour/)** and authenticate to various services (*SSH, Git*).
  - [Sign](/solutions/signature/) your documents and [emails](/solutions/theme-email/).
  - [Encrypt and decrypt](/solutions/encryption/) your documents and [emails](/solutions/theme-email/).
  - [Certify](/solutions/theme-authentication/) your peers and extend your [Web of Trust](https://en.wikipedia.org/wiki/Web_of_trust).
  - Verify the signature of documents and emails.
- **Bonus**: check your **[djis (Ɉ)](/about/join/)** accounts.

#### --- 19:00 --- Shared **meal**

---

### Registration

The workshop requires preparation (tables, plugs, computers), so places are limited. **The precise address in Tallard will be sent to you by SMS after registration.**

**To register, please text** [07 56 83 22 15](tel:+33756832215). Example:
```
Hello, I'd like [x] seat(s) for [the workshop|the talk|the talk and the workshop] of 5 June.
```

Looking forward to meeting you! :-)

---
