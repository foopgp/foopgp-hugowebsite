...
These security keys allow the use of the private keys of your [digital identity](/fr/solutions/pgp-id) without allowing any direct access to them. Each use may require a PIN code. After three consecutive incorrect PIN attempts, the key locks. It can be unlocked with another code (PUK); otherwise, it erases its data. You will then need to reset it to restart the QR code scanning.
...
[NitroKeys](https://www.nitrokey.com/) offer a comprehensive and secure solution to protect [digital identities](/fr/solutions/pgp-id) and sensitive data. Their use significantly enhances the security of systems and information while remaining practical and accessible to users. They are almost identical to the [YubiKey](./#YubiKey).
